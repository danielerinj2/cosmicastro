...
from app.ui.components import app_header, auth_sidebar, confidence_banner, parse_time_ampm, premium_upgrade_block
...
with st.expander("Add new partner profile"):
    ...
    if not has_birth_time:
        st.session_state["partner_birth_time_ampm"] = ""
    birth_time = None
    if has_birth_time:
        birth_time_raw = st.text_input(
            "Birth time (AM/PM)",
            key="partner_birth_time_ampm",
    ...
    add_partner = st.button("Save Partner")

    if add_partner:
        if not name.strip():
            st.error("Partner name is required.")
        elif has_birth_time and birth_time is None:
            ...
            partner = service.repo.create_partner_profile(
                ...
                birth_time=birth_time.isoformat() if (has_dob and birth_time) else None,
